...
def drop_table(con, cs, table, ignore_errors=False):
    try:
        cs.execute('drop table %s' % tables[table])
    except:
...
class Base(unittest.TestCase):
    ...
    def _insert(self, cur, table=None, data=None, stmt_type='insert'):
        ...
        if self.driver.paramstyle == 'qmark':
            cur.execute(stmt_base + '(?)', data.values())
        elif self.driver.paramstyle == 'numeric':
            cur.execute(stmt_base + '(:1)', data.values())
        elif self.driver.paramstyle == 'format':
            cur.execute(stmt_base + '(%%s)', data.values())
        elif self.driver.paramstyle == 'named':
            cur.execute(stmt_base + '(:beer)', data)
        elif self.driver.paramstyle == 'pyformat':
            cur.execute(stmt_base + '(%%(beer)s)', data)
        else:
...
class TestConnection(Base):
    ...
    def test_close(self):
        ...
        if self.driver_name != 'pysqlite2':
          self.assertRaises(self.driver.Error,con.execute(qry))
        else:
          self.assertRaises(self.driver.ProgrammingError,con.execute(qry))
        self.assertRaises(self.driver.Error,con.commit)
        self.assertRaises(self.driver.Error,con.close)
...
class TestCursor(Base): 
    ...
    def test_description(self):
        ...
        drop_table(con, cur, 'booze', ignore_errors=True)
        drop_table(con, cur, 'barflys', ignore_errors=True)
        try:
    ...
    def test_insert(self):
        con, cur = connect_plus_cursor()
        self._insert(cur, data=dict(name="Cooper's"))
        self._insert(cur, data=dict(name="Victoria Bitter"))
        cur.execute('select name from %sbooze' % table_prefix)
        res = cur.fetchall()
        ls = len(res)
...
class TestTypesEmbedded(Base):
    ...
    def _get_lastrow(self):
        ...
        #SQLITE uses a property for lastrowid
        id_value = self.cs.lastrowid
        self._insert(self.cs, table=self.table, data=dict(int1=id_value), stmt_type='select')
    ...
    def test_date_insert_string(self):
        ...
        self._insert(self.cs, table=self.table, data=data)        
        self.assertEqual(self._get_lastrow()[2], date1)
    ...
    def test_date_insert(self):
        ...
        self._insert(self.cs, table=self.table, data=data)
        self.assertEqual(self._get_lastrow()[2], date1_string)
    ...
    def test_timestamp_insert_string(self):
        ...
        self._insert(self.cs, table=self.table, data=dict(timestamp1=timestamp1))
    ...
    def test_timestamp_insert(self):
        ...
        self._insert(self.cs, table=self.table, data=dict(timestamp1=timestamp1))
        self.assertEqual(self._get_lastrow()[3],timestamp1_string)
        #self.assertEqual(self._get_lastrow()[3],timestamp1)

    def test_time_insert_string(self):
        time1 = '11:03:13'
        time1_time = datetime.time(11, 03, 13)
        self._insert(self.cs, table=self.table, data=dict(time1=time1))
        #self.assertEqual(self._get_lastrow()[4],time1_time)
        self.assertEqual(self._get_lastrow()[4],time1)
    ...
    def test_time_insert_subsecond_string(self):
        time1 = '11:03:13.9999'
        self.cs.execute('insert into %s (time1) values(?)' % self.tableName, time1)
    ...
    def test_callproc(self):
        ...
        try:
            ...
            if driver_supports.lower_func and hasattr(cur,'callproc'):
                r = cur.callproc(self.driver_supports.lower_func,('FOO',))
                self.assertEqual(len(r),1)
